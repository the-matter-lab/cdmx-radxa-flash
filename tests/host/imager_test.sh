...
assert_eq 98 "$(validate_team 98; printf 98)" 'future team 98 is valid'
assert_eq admin "$(validate_team admin; printf admin)" 'admin identity is valid'
assert_fails 'numeric team 99 is reserved for admin' validate_team 99
assert_fails 'team 100 is rejected' validate_team 100
...
write_team_config "$tmp" 98
assert_eq CDMX_HOSTNAME=equipo98 "$(grep '^CDMX_HOSTNAME=' "$tmp/cdmx-team.env")" 'future team marker contains hostname'
...
CDMX_ROOT="$limits_root" "$ROOT/device/apply-resource-limits.sh" 98
assert_eq MemoryMax=256M \
  "$(grep '^MemoryMax=' "$limits_root/etc/systemd/system/cdmx-picoclaw.service.d/20-memory.conf")" \
  'future team 98 receives the lightweight agent memory profile'
...
assert_fails 'resource limits reject an unknown team' env CDMX_ROOT="$limits_root" \
  "$ROOT/device/apply-resource-limits.sh" 99
